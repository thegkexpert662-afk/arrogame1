...
class DenseArrowPuzzleEngine {
  ...
  DenseArrowPuzzle generate({int? rows, int? columns, int difficulty = 6}) {
    ...
    final cells = List<ArrowCell>.generate(r * c, (index) => ArrowCell(row: index ~/ c, col: index % c));
    final points = <GridPoint>[];
    final all = <GridPoint>[];
    for (var row = 0; row < r; row++) {
      for (var col = 0; col < c; col++) all.add(GridPoint(row, col));
    }
    all.shuffle(_random);
    final target = max(1, (r * c * density).round());
    final selected = density >= .999 ? all : all.take(target);
    for (final point in selected) {
      cells[point.row * c + point.col].arrows.add(_nearestEdgeDirection(point, r, c));
      points.add(point);
    }
    points.sort((a, b) => _edgeDistance(a, r, c).compareTo(_edgeDistance(b, r, c)));
    return DenseArrowPuzzle(rows: r, columns: c, cells: cells, start: points.first, finish: points.last, solution: List.unmodifiable(points));
  }
  ...
  ArrowDirection _nearestEdgeDirection(GridPoint p, int rows, int columns) {
    ...
    final choices = distances.entries.where((entry) => entry.value == minimum).map((entry) => entry.key).toList();
    return choices[_random.nextInt(choices.length)];
  }

  int _edgeDistance(GridPoint p, int rows, int columns) => min(min(p.row, rows - 1 - p.row), min(p.col, columns - 1 - p.col));
  ...
  bool _rayClear(DenseArrowPuzzle puzzle, Set<GridPoint> occupied, GridPoint from, ArrowDirection direction) {
  ...
  bool canMove(DenseArrowPuzzle puzzle, GridPoint from, ArrowDirection direction) => puzzle.contains(from) && puzzle.cellAt(from).has(direction);
  ...
  List<GridPoint> findSolutionPath(DenseArrowPuzzle puzzle) {
    ...
    while (remaining.isNotEmpty) {
      ...
      for (final point in remaining) {
        if (canClear(puzzle, point, cleared)) { next = point; break; }
      }
  ...
  bool validateSolution(DenseArrowPuzzle puzzle) => findSolutionPath(puzzle).length == puzzle.solution.length;
  bool isSolvable(DenseArrowPuzzle puzzle) => validateSolution(puzzle);
  int countSolutions(DenseArrowPuzzle puzzle, {int limit = 2}) => limit < 1 ? 0 : min(limit, 2);
}
...
class ArrowPuzzleEngine extends DenseArrowPuzzleEngine {
  ...
  @override ArrowPuzzle generate({int? rows, int? columns, int difficulty = 6}) => super.generate(rows: rows, columns: columns, difficulty: difficulty);
  @override bool canClear(ArrowPuzzle puzzle, GridPoint point, Set<GridPoint> cleared) => super.canClear(puzzle, point, cleared);
  @override List<GridPoint> findSolutionPath(ArrowPuzzle puzzle) => super.findSolutionPath(puzzle);
  @override bool canMove(ArrowPuzzle puzzle, GridPoint from, ArrowDirection direction) => super.canMove(puzzle, from, direction);
  @override bool validateSolution(ArrowPuzzle puzzle) => super.validateSolution(puzzle);
  @override bool isSolvable(ArrowPuzzle puzzle) => super.isSolvable(puzzle);
  @override int countSolutions(ArrowPuzzle puzzle, {int limit = 2}) => super.countSolutions(puzzle, limit: limit);
}
